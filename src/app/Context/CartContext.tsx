...
// ✅ Cart Item Type (id should be string)
interface CartItem {
  id: string;  // ✅ Sanity CMS IDs are always strings
  name: string;
  price: number;
  imageUrl: string;
  quantity: number;
...
interface CartContextType {
  ...
  cartQuantity: number;
  addToCart: (product: CartItem, quantity: number) => void;
  removeFromCart: (id: string) => void;  // ✅ Accepts string id
  clearCart: () => void;
}
...
export const CartProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  // ✅ Function to Remove Item from Cart
  const removeFromCart = (id: string) => {  // ✅ Now accepts string id
    const updatedCart = cart.filter((item) => item.id !== id);
    setCart(updatedCart);
    setCartQuantity(updatedCart.reduce((acc, item) => acc + item.quantity, 0));
    localStorage.setItem("cart", JSON.stringify(updatedCart));
